---
title: random
description: 乱数生成と確率分布。
sidebar:
  order: 4
---

`random` モジュールは乱数生成、確率分布、シーケンスユーティリティを提供します。

```purus
use random as r
```

## コア

### `random[]`
[0, 1] の範囲の浮動小数点数を返します。

```purus
use random as r
const x be r.random[]  -- 例: 0.7312...
```

### `randint[a; b]`
`a` から `b`（両端を含む）のランダムな整数を返します。

```purus
use random as r
r.randint[1; 6]  -- 例: 4（サイコロ）
```

### `randrange[stop]` / `randrange[start; stop]` / `randrange[start; stop; step]`
範囲からランダムな整数を返します。引数1つの場合は [0, stop)。2つの場合は [start, stop)。3つの場合は `start, start+step, start+2*step, ...` のうち `stop` 未満のランダムな値。

```purus
use random as r
r.randrange[10]        -- 0..9
r.randrange[5; 10]     -- 5..9
r.randrange[0; 20; 2]  -- 偶数 0..18
```

### `randbool[]` / `randbool[p]`
確率 `p` で `true` を返します（デフォルト: 0.5）。

```purus
use random as r
r.randbool[]      -- 50% true
r.randbool[0.8]   -- 80% true
```

### `getrandbits[k]`
`k` ビットのランダムな整数を返します。

```purus
use random as r
r.getrandbits[8]   -- 0..255
r.getrandbits[16]  -- 0..65535
```

### `randbytes[n]`
`n` 個のランダムなバイト（0–255）の配列を返します。

```purus
use random as r
r.randbytes[4]  -- 例: [142, 55, 200, 13]
```

### `normalvariate[mu; sigma]`
`gauss` のエイリアス。正規分布からランダムな数を返します。

## 実数値分布

### `uniform[a; b]`
[a, b) の範囲のランダムな浮動小数点数を返します。

```purus
use random as r
r.uniform[1.0; 5.0]  -- 例: 3.271...
```

### `triangular[lo; hi; mode]`
三角分布からランダムな数を返します。すべてのパラメータは省略可能（デフォルト: lo=0, hi=1, mode=中間点）。

### `gauss[mu; sigma]`
平均 `mu`、標準偏差 `sigma` のガウス分布（正規分布）からランダムな数を返します。両方とも省略可能（デフォルト: mu=0, sigma=1）。

```purus
use random as r
r.gauss[0; 1]       -- 標準正規分布
r.gauss[100; 15]    -- IQ風の分布
```

### `expovariate[lambd]`
レート `lambd` の指数分布からランダムな数を返します。

### `gammavariate[alpha; beta]`
ガンマ分布からランダムな数を返します。

### `betavariate[alpha; beta]`
[0, 1] のベータ分布からランダムな数を返します。

### `lognormvariate[mu; sigma]`
対数正規分布からランダムな数を返します。

### `vonmisesvariate[mu; kappa]`
フォン・ミーゼス分布（円形正規分布）からランダムな角度を返します。`mu` はラジアン単位の平均角度、`kappa` は集中度パラメータです。

### `paretovariate[alpha]`
パレート分布からランダムな数を返します。

### `weibullvariate[alpha; beta]`
ワイブル分布からランダムな数を返します。

## シーケンス

### `choice[arr]`
配列からランダムな要素を1つ返します。

```purus
use random as r
r.choice[[///red///; ///green///; ///blue///]]  -- 例: "green"
```

### `choices[arr; k]`
`k` 個のランダムな要素の配列を返します（重複あり）。

```purus
use random as r
r.choices[[///a///; ///b///; ///c///]; 5]  -- 例: ["b", "a", "c", "a", "b"]
```

### `wchoices[arr; weights; k]`
重み付きでランダムに `k` 回選択します。各要素の確率はその重みに比例します。

```purus
use random as r
r.wchoices[[///rare///; ///common///]; [1; 9]; 10]
-- ほとんど "common"
```

### `shuffle[arr]`
要素をランダムな順序にした新しい配列を返します（元の配列は変更しません）。

```purus
use random as r
r.shuffle[[1, 2, 3, 4, 5]]  -- 例: [3, 1, 5, 2, 4]
```

### `sample[arr; k]`
配列から重複なしで `k` 個のランダムな要素を返します。

```purus
use random as r
r.sample[[1, 2, 3, 4, 5]; 3]  -- 例: [4, 1, 3]
```

## 離散分布

### `binomial[n; p]`
二項分布からランダムな数を返します。確率 `p` で `n` 回の試行における成功回数です。

```purus
use random as r
r.binomial[10; 0.5]  -- 例: 6
```

### `poisson[lambda]`
期待値 `lambda` のポアソン分布からランダムな数を返します。

### `geometric[p]`
幾何分布から返します。確率 `p` で最初の成功までに必要な試行回数です。

## ユーティリティ

### `clamp[val; lo; hi]`
`val` を [lo, hi] の範囲に制限して返します。

```purus
use random as r
r.clamp[15; 0; 10]   -- 10
r.clamp[-5; 0; 10]   -- 0
r.clamp[5; 0; 10]    -- 5
```

### `lerp[a; b; t]`
`a` と `b` の間を係数 `t`（0–1）で線形補間します。

```purus
use random as r
r.lerp[0; 100; 0.5]   -- 50
r.lerp[0; 100; 0.25]  -- 25
```
