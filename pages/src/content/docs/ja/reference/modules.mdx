...
`use` キーワードでPurus組み込みの標準ライブラリモジュールをインポートできます。`as` キーワードでバインディング名を指定する必要があります。ツリーシェイキングにより、実際に使用した関数のみがコンパイル出力に含まれます。
...
| `math` | JS `Math` エイリアス＋小文字の定数エイリアス（`pi`, `e`, `ln2`, `ln10`, `log2e`, `log10e`, `sqrt2`, `sqrt1_2`） |
