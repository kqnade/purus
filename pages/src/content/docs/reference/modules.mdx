...
The `use` keyword imports Purus built-in standard library modules. The `as` keyword is required to specify the binding name. Tree-shaking ensures only the functions you actually reference are included in the compiled output.
